...
import progressbar
from ..trend.functions import getPennyStocks
...
def scan_trends(rescan=False):
    ...
    if (rescan):
        ...
        with progressbar.ProgressBar(max_value=len(tickers), prefix='Request: ', redirect_stdout=True) as bar:
...
def print_trends():
    recent_interest = []
    for t in progressbar.progressbar(tickers):
