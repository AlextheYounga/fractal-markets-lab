...
def collectOptionChain(ticker, dummyData):
    ...
    four_months_away = (today + relativedelta(months=+4))    
    four_months_away_days = calendar.monthrange(four_months_away.year, four_months_away.month)[1]

    # Building a timerange to send to TD Ameritrade's API
    fromDate = today
    toDate = datetime.datetime(four_months_away.year, four_months_away.month, four_months_away_days)
    ...
    if (chain):
        ...
        for optionSide in ['callExpDateMap', 'putExpDateMap']:
            for expir, strikes in chain[optionSide].items():  # Looping each side of the option chain
                ...
                if (daysToExpiration > 21):  # Must be at least 7 days from expiration.
        ...
        def vixExpirationRules(options):
            ...
            won't be very useful for anything farther in the future. 
            """
            ...
            for side, option in options.items():
                for exp, data in option.items():
                    dte = data['dateInfo']['daysToExpiration']
                    if (dte <= 30):
                        tradeStrikes.append(exp)
                    if (dte > 90 and dte <= 120 ):
                        trendStrikes.append(exp)
                

            if (len(tradeStrikes) != 0 and len(trendStrikes) != 0):
                nearTermExp = min(tradeStrikes)
                nextTermExp = min(trendStrikes)
